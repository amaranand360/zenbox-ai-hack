        ...
        <div className="max-w-5xl mx-auto w-full space-y-12 sm:space-y-16">
            ...
            <form onSubmit={handleSearch} className="relative">
              <div className="relative group">
                <input
                  ...
                  className="w-full bg-[#2a2a2a] border border-[#444] rounded-2xl px-6 py-5 pr-32 text-white placeholder-gray-400 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-all text-lg group-hover:border-[#555]"
                />
                <div className="absolute right-4 top-1/2 transform -translate-y-1/2 flex items-center gap-3">
                  <button
                    type="button"
                    className="p-2.5 hover:bg-[#3a3a3a] rounded-xl transition-colors"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                    </svg>
                  </button>
                  <div className="flex items-center gap-2 bg-[#3a3a3a] rounded-xl px-3 py-2 border border-[#555]">
          ...
          <div className="flex flex-wrap gap-4 mb-16 justify-center">
            {suggestedPrompts.map((prompt, index) => (
              <button
                ...
                className="flex items-center gap-3 bg-[#2a2a2a] hover:bg-[#3a3a3a] border border-[#444] hover:border-purple-500/50 rounded-2xl px-6 py-3 text-sm transition-all duration-200 group"
              >
                <span className="text-lg">{prompt.icon}</span>
                <span className="group-hover:text-purple-300 transition-colors">{prompt.text}</span>
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
            {quickActions.map((action, index) => (
              <div
                key={index}
                className="bg-[#1e1e1e] border border-[#333] rounded-2xl p-8 hover:border-[#444] hover:bg-[#252525] transition-all duration-200 cursor-pointer group"
              >
                <div className="flex items-center justify-between mb-6">
                  <div className={`w-16 h-16 bg-gradient-to-r ${action.gradient} rounded-2xl flex items-center justify-center text-3xl shadow-lg`}>
                    {action.icon}
                  </div>
                  {action.status && (
                ...
                {action.subtitle && (
                  <p className="text-sm text-gray-400 leading-relaxed">{action.subtitle}</p>
                )}
              </div>
            ))}
          </div>
